...
legacy LLM price table per-1M) into one per-1M registry, and supports an external
JSON/YAML override file so deployments can adjust prices without code changes. The
LLM cost helper delegates here.
"""
...
# claude-3-5-sonnet and claude-3-haiku match the legacy LLM price table so
# existing estimate_llm_cost() callers are unaffected.
